...
namespace EvolveGames
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            isRunning = !isCrough ? CanRunning ? ControlFreak2.CF2Input.GetKey(KeyCode.LeftShift) : false : false;
            vertical = canMove ? (isRunning ? RunningValue : WalkingValue) * ControlFreak2.CF2Input.GetAxis("Vertical") : 0;
            horizontal = canMove ? (isRunning ? RunningValue : WalkingValue) * ControlFreak2.CF2Input.GetAxis("Horizontal") : 0;
            ...
            if (ControlFreak2.CF2Input.GetButton("Jump") && canMove && characterController.isGrounded && !isClimbing)
            {
            ...
            else if (!Moving && playsound)
            {
                playsound = false;
                FootSteps.Stop();
            }

            if (ControlFreak2.CFCursor.lockState == CursorLockMode.Locked && canMove)
            {
                Lookvertical = -ControlFreak2.CF2Input.GetAxis("Mouse Y");
                Lookhorizontal = ControlFreak2.CF2Input.GetAxis("Mouse X");
            ...
            if (ControlFreak2.CF2Input.GetKey(CroughKey))
            {
                isCrough = true;
        ...
        private void OnTriggerStay(Collider other)
        {
            if (other.tag == "Ladder" && CanClimbing)
            {
                moveDirection = new Vector3(0, ControlFreak2.CF2Input.GetAxis("Vertical") * Speed * (-Camera.localRotation.x / 1.7f), 0);
            }
        }
